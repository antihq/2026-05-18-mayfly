...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-900 antialiased text-zinc-950 dark:text-white">
        <flux:sidebar sticky collapsible="mobile" class="bg-white lg:bg-zinc-50 dark:bg-zinc-900 border-r border-zinc-950/5 dark:border-white/5">
            ...
            <flux:sidebar.nav>
                <flux:sidebar.item icon="inbox" :href="route('entries.index')" :current="request()->routeIs('entries.*')" :accent="false" wire:navigate>
                    Entries
                </flux:sidebar.item>
                <flux:sidebar.item icon="clock" :href="route('archived-entries')" :current="request()->routeIs('archived-entries')" :accent="false" wire:navigate>
                    Archive
                </flux:sidebar.item>
            </flux:navbar>
